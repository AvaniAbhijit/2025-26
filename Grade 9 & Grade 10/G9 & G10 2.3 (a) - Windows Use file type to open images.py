# On line 21 - mark_attendance() function
# 1. allows the user to select an image file using a file dialog.
# 2. It uses filedialog.askopenfilename() to open a file selection window restricted
#    to image formats (.png, .jpg, .jpeg).
# 3. The selected file path is stored in the variable image_file_path.

# Task 1 : Run the code and check the ouptut.
# Task 2 : Print the selected image path on line 23.
...
def mark_attendance():#defining function to open images
    image_file_path = filedialog.askopenfilename(filetypes=[("Image files", "*.png;*.jpg;*.jpeg")])#opening image files
